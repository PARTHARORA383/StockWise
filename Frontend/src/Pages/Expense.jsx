...
const Expenses = () => {
  ...
  const handlefetchexpense = async () => {
    ...
    const response = await axios.get(`${process.env.REACT_APP_BACKEND_BASE_URL}/Expense/${uid}/${companyid}`)
  ...
  const handlePostExpenst = async ()=>{
    ...
    try{
      ...
      const response = await axios.post(`${process.env.REACT_APP_BACKEND_BASE_URL}/Expense/${uid}/${companyid}` , requestbody , {
      headers : {
